...
from mainapp.models import Album, Photo
...
def logged_in_client(db, django_user_model, client):
    user = django_user_model.objects.create_user(username="newuser", password="newpassword")
...
def existing_albums(db, logged_in_client):
    def create_album():
        ...
        for album_name in existing_albums:
            Album.objects.create(album_name=album_name, author=logged_in_client, num_of_photos=num_of_photos)
            num_of_photos += 1
