...
class AlbumCreationSerializer(serializers.ModelSerializer):
    ...
    def custom_validate(self, validated_data, user):
        try:
            Album.objects.get(album_name=validated_data.get("album_name"), author = user).pk
            raise serializers.ValidationError({"detail": "Album with this name already exists"})
        except:
            pass
...
class AlbumNameSerializer(serializers.Serializer):
    ...
    def custom_validate(self, validated_data, user):
        try:
            ...
            raise serializers.ValidationError({"detail": "Album with this name does not exist"})
...
class GetAlbumListSerializer(serializers.Serializer):
    ...
    def custom_validate(self, validated_data):
        ...
        if sort_param not in ["date_created", "num_of_photos", ""]:
            raise serializers.ValidationError({"detail": "Unknown sort parameter. Please, "+
            "provide 'date_created' or 'num_of_photos' or leave it blank"})
        elif sort_param in ["date_created", "num_of_photos"]:
            if sort_order not in ["+","-"]:
                raise serializers.ValidationError({"detail": "Unknown sort order. "+
                "Please, provide '+' or '-'"})
...
class PhotoCreationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        existing_names = list(existing_photos.values_list('photo_name', flat=True))
        if validated_data.get("photo_name") in existing_names:
            raise serializers.ValidationError({"detail": "Photo with this name in this album already exists"})
        else:
...
class PhotoSerializer(serializers.ModelSerializer):
    ...
    #def response_album_name(self, validated_data):


    class Meta:
        model = Photo
        fields = ('photo_name', 'date_added', 'file', 'tags', 'album_name')
...
class PhotoBasicSerializer(serializers.Serializer):
    ...
    def custom_validate(self, validated_data, user):
        ...
        if validated_data.get('photo_name') not in existing_names:
            raise serializers.ValidationError({"detail": "Photo with this name does not exist"})
...
class PhotoEditSerializer(serializers.Serializer):
    ...
    def update(self, validated_data):
        ...
        if validated_data.get("new_photo_name") in existing_names:
            ...
            photo = Photo.objects.get(album=album.pk, photo_name=validated_data.get("photo_name"))
            if tags != "":
    ...
    def custom_validate(self, validated_data, user):
        ...
        if new_photo_name == "" and tags == "":
            raise serializers.ValidationError({"detail": "You should update either photo_name or tags"})
...
class PhotoListSerializer(serializers.Serializer):
    ...
    def custom_validate(self, validated_data, user):
        ...
        if sort_param not in ["date_added", "album", ""]:
            raise serializers.ValidationError({"detail": "Unknown sort parameters. Please, "+
            "provide 'date_added' or 'album' or leave it blank"})
        elif sort_param in ["date_added", "album"]:
            if sort_order not in ["+","-"]:
                raise serializers.ValidationError({"detail": "Unknown sort order. "+
                "Please, provide '+' or '-'"})
        if filter_param not in ["tags","albums","tags, albums","albums, tags", ""]:
            raise serializers.ValidationError({"detail": "Unknown filter parameters. Please, "+
            "provide 'tags' or 'albums' or 'tags, albums' leave it blank"})
        ...
        author_albums = list(Album.objects.filter(author=user).values_list('album_name', flat=True))
        if filter_param == "albums" or filter_param == "tags, albums" or filter_param == "albums, tags":
            if albums_parsed == []:
                raise serializers.ValidationError({"detail": "No albums were provided"})
            for album in albums_parsed:
                if album not in author_albums:
                    raise serializers.ValidationError({"detail": "Some albums that you provided "+
                    "do not exist."})
        if filter_param == "tags" or filter_param == "tags, albums" or filter_param == "albums, tags":
            if tags_parsed == []:
                raise serializers.ValidationError({"detail": "No tags were provided"})
